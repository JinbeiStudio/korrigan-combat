...
$app->get('/api/1.0/combat-tour1/attaquant/{idJoueur}/defenseur/{idDefenseur}', function ($req, $resp, $args) {
    ...
    try {
        ...
        //Si il y a un gagant on ajoute l'or et l'expérience
        if ($idGagnant != NULL) {
...
/* ----------------------- Combat Tour 2 --------------------------------------- */
/* Endpoint : /api/1.0/combat-tour2/attaquant/{idJoueur}/defenseur/{idDefenseur} */
/* ------------------------------ Method : GET --------------------------------- */
/* ------------------------- Auteur : Julien Gabriel --------------------------- */
$app->get('/api/1.0/combat-tour2/{idCombat}', function ($req, $resp, $args) {
    global $__player_id;

    $idCombat = $args['idCombat'];
    $players = getPlayers($idCombat);

    $checkCombatExist = checkCombatExist($idCombat);

    if ($checkCombatExist == FALSE) {
        __log('Combat inexistant');
        return $resp->withStatus(400);
    }

    if ($players == FALSE) {
        __log('Impossible de trouver les joueurs');
        return $resp->withStatus(400);
    }

    $idJoueur = $players[0]['idAttaquant'];
    $idDefenseur = $players[0]['idDefenseur'];
    ...
    if ($statsTour1 == FALSE) {
        __log('Or et expérience du tour 1 introuvable');
        return $resp->withStatus(400);
    }
    ...
    //Or total gagné pour l'attaquant et le défenseur
    ...
    $experienceDefenseurTour1 = $statsTour1[0]["experienceDefenseur"];

    //Experience éventuellement gagné au tour 2 pour le joueur
    $experienceAttaquantTour2 = experienceGagne($attaquerDefenseur);
    $experienceDefenseurTour2 = experienceGagne($attaquerAttaquant);

    //Expérience totale gagnée
    $experienceAttaquant = $experienceAttaquantTour1 + $experienceAttaquantTour2;
    $experienceDefenseur = $experienceDefenseurTour1 + $experienceDefenseurTour2;

    //Détermine la gagnant au deuxième tour
    if ($attaquerAttaquant['perdu'] === true || $attaquerAttaquant['perdu'] === true && $attaquerDefenseur['perdu'] === true) {
        $idGagnant = $idDefenseur;
        $idPerdant = $idJoueur;
        $orGagne = $orDefenseur;
    ...
    try {
        ...
        foreach ($attaquerAttaquant as $key => $value) {
            if ($key != "perdu") {
                $stmt = $pdo->prepare('INSERT INTO resultatsTours (idCombat, numeroTour, idTroupeJoueur, quantitePerdues) VALUES (:idCombat, 2, :idTroupeJoueur, :quantitePerdues)');
                $stmt->execute(["idCombat" => $idCombat, "idTroupeJoueur" => $key, "quantitePerdues" => $value['quantitePerdue']]);
            }
        }
        /* -------------------------------------------------------------------------- */

        /* ----------- Insertion des résultats du tour 2 pour le défenseur ---------- */
        foreach ($attaquerDefenseur as $key => $value) {
            if ($key != "perdu") {
                $stmt = $pdo->prepare('INSERT INTO resultatsTours (idCombat, numeroTour, idTroupeJoueur, quantitePerdues) VALUES (:idCombat, 2, :idTroupeJoueur, :quantitePerdues)');
                $stmt->execute(["idCombat" => $idCombat, "idTroupeJoueur" => $key, "quantitePerdues" => $value['quantitePerdue']]);
        ...
        $ret = array(
            'resultatTour2Attaquant' => (array) $items,
        );

        if (!$ret) {
            __log('Erreur lors du calcul du tour 2');
            return $resp->withStatus(404);   // Not found
        }
        return buildResponse($resp, $ret);
    } catch (Exception $e) {
        $pdo->rollback();
        __logException('Erreur lors du calcul du tour 2', $e);
        return $resp->withStatus(500);   // Internal Server Error
    }
});
/* -------------------------------------------------------------------------- */
...
/*                               Id des joueurs                               */
/* -------------------------------------------------------------------------- */
function getPlayers($idCombat)
{
    try {
        $pdo = getPDO();

        //Id du joueur attaquant
        $queryID = $pdo->prepare("SELECT `idAttaquant`, `idDefenseur` FROM `combats` WHERE idCombat = :idCombat");
...
/*                             Or gagné au tour 1                             */
/* -------------------------------------------------------------------------- */
function statsTour1($idAttaquant, $idDefenseur, $idCombat)
{
    try {
        $pdo = getPDO();
...
/* -------------------------------------------------------------------------- */
/*           Vérification de l'existence du combat                            */
/* -------------------------------------------------------------------------- */
function checkCombatExist($idCombat)
{
    try {
        $pdo = getPDO();
